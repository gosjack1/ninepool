...
'''
config_file = resolve_config_path('arm.yaml')
...
strategy_weights_file = resolve_config_path('strategy_weights.yaml')
with open(strategy_weights_file, 'r') as file:
    _strategy_weights = yaml.safe_load(file)

DEFAULT_ANGLE_WEIGHT = _strategy_weights['angle_weight']
DEFAULT_DIST_WEIGHT = _strategy_weights['dist_weight']
DEFAULT_N_WEIGHT = _strategy_weights['n_weight']
DEFAULT_SCORE_OFFSET = _strategy_weights['score_offset']
...
def simple_route(cue=[0, 0], cuetoivector=[0, 0], itok2vector=[0, 0] ,k2tok1vector=[0, 0], toholevector=[0, 0], n=0, angle_weight=DEFAULT_ANGLE_WEIGHT, dist_weight=DEFAULT_DIST_WEIGHT,  n_weight=DEFAULT_N_WEIGHT, score_offset=DEFAULT_SCORE_OFFSET, return_breakdown=False):
    #fix cuefinalvector
    score = None
    angle_score = 0.0
    dist_score = 0.0
    ...
    if n == 0 or n == -1:
        if n == 0:
            ...
            angle = angle_between_vector(cuetoivector, toholevector)
            angle_score = -angle*angle_weight + score_offset
            dist_score = -dist_weight*(cuetoiL+itohL) + score_offset
            n_score = -n*n_weight + score_offset
            score = angle_score + dist_score + n_score
        else:
            cuefinalvector = [cuetoivector[0]-toholevector[0], cuetoivector[1]-toholevector[1]]
            cuetoiL = math.sqrt(abs(cuetoivector[0])**2+abs(cuetoivector[1])**2)
            itohL = math.sqrt(abs(toholevector[0])**2+abs(toholevector[1])**2)
            dotproduct0 = cuetoivector[0]*toholevector[0] + cuetoivector[1]*toholevector[1]
            angle_score, dist_score, n_score = -6000.0, 0.0, 0.0
            score = -6000.0
        ...
        angle0 = angle_between_vector(cuetoivector, itok2vector)
        angle1 = angle_between_vector(itok2vector, toholevector)
        angle_score = (-angle0*angle_weight + score_offset)/2 + (-angle1*angle_weight + score_offset)/2
        dist_score = -dist_weight*(cuetoiL+itok2L+k2toholeL) + score_offset
        n_score = -n*n_weight + score_offset
        score = angle_score + dist_score + n_score
        ...
        angle0 = angle_between_vector(cuetoivector, itok2vector)
        angle1 = angle_between_vector(itok2vector, toholevector)
        angle2 = angle_between_vector(k2tok1vector, toholevector)
        angle_score = (-angle0*angle_weight + score_offset)/3 + (-angle1*angle_weight + score_offset)/3 + (-angle2*angle_weight + score_offset)/3
        dist_score = -dist_weight*(cuetoiL+itok2L+k2tok1L+k1toholeL) + score_offset
        n_score = -n*n_weight + score_offset
        score = angle_score + dist_score + n_score
    final_hitpoint_x, final_hitpoint_y = hitpoint(cue[0], cue[1], cuetoivector[0], cuetoivector[1])
    breakdown = {"angle_score": angle_score, "dist_score": dist_score, "n_score": n_score}
    if return_breakdown:
        return score, cuetoivector, [final_hitpoint_x, final_hitpoint_y], breakdown
...
def reflected_route(cue, cue_to_wall_v, wall_to_target_v, target_to_aim_v, angle_weight=DEFAULT_ANGLE_WEIGHT, dist_weight=DEFAULT_DIST_WEIGHT, score_offset=DEFAULT_SCORE_OFFSET):
    ...
    score = (-angle0*angle_weight + score_offset)/2 + (-angle1*angle_weight + score_offset)/2 + (-dist_weight*(cue_to_wall_dis+wall_to_target_dis+target_to_aim_dis) + score_offset)
...
def select_target_ball(ball_numbers):
    """
    Nine-ball rule: the only legal object ball is the lowest-numbered ball
    still on the table. Given the list of ball numbers currently on the table,
    return the number of the ball that must be contacted first.

    Returns None when the list is empty (no balls left to shoot).
    """
    legal = [n for n in ball_numbers if n is not None]
    if not legal:
        return None
    return min(legal)


def order_target_first(objectballx, objectbally, ball_numbers):
    """
    Reorder the object-ball coordinate arrays (and their parallel ball-number
    list) so that the legal nine-ball target -- the lowest-numbered ball on the
    table -- sits at index 0. The routing algorithm in main() always treats
    index 0 as the ball to pot, so this is what actually enforces the
    lowest-number-first rule instead of blindly shooting whatever the vision
    system happened to list first.

    objectballx / objectbally are parallel coordinate arrays; ball_numbers is a
    parallel list holding the printed number of each object ball. All three must
    be the same length and are mutated in place. Returns
    (objectballx, objectbally, ball_numbers, target_number); target_number is
    None (and nothing is reordered) when no ball numbers are supplied.
    """
    if not ball_numbers:
        return objectballx, objectbally, ball_numbers, None

    target_number = select_target_ball(ball_numbers)
    if target_number is None:
        return objectballx, objectbally, ball_numbers, None

    target_index = ball_numbers.index(target_number)
    if target_index != 0:
        objectballx.insert(0, objectballx.pop(target_index))
        objectbally.insert(0, objectbally.pop(target_index))
        ball_numbers.insert(0, ball_numbers.pop(target_index))
    return objectballx, objectbally, ball_numbers, target_number


def main(objectballx, objectbally, cuex, cuey, ball_numbers=None) -> Tuple:
    ...
    # Nine-ball rule: the legal target is the lowest-numbered ball on the table,
    # not simply whatever happens to sit at index 0. When the caller supplies the
    # parallel ball-number list, move that ball to index 0 so all of the routing
    # below (which hard-codes objectballx[0]/objectbally[0] as the target) aims at
    # the correct ball. Without ball_numbers we fall back to the legacy behaviour
    # of trusting the upstream ordering.
    if ball_numbers is not None:
        objectballx, objectbally, ball_numbers, target_number = order_target_first(
            objectballx, objectbally, ball_numbers)
        print("nine-ball legal target (lowest number on table):", target_number)
