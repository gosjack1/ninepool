...
import hiwin_control.hiwin_control.nine_ball_strat_old as strat
...
def show_aim_ui(
    ...
    dict  with keys:
        ...
        hit_strength – 0=Soft, 1=Mid, 2=Mid-Heavy, 3=Heavy
    """
    ...
    # Which object ball the cue should hit (click on the canvas ball to change)
    target_ball_var = tk.IntVar(value=0)

    def _on_canvas_click(event):
        """Select the nearest object ball as the hit target."""
        best_idx, best_dist = 0, float('inf')
        for i, (bx, by) in enumerate(zip(object_ball_x, object_ball_y)):
            cx_, cy_ = a2c(bx, by)
            dist = math.sqrt((event.x - cx_) ** 2 + (event.y - cy_) ** 2)
            if dist < best_dist:
                best_dist, best_idx = dist, i
        if best_dist < BALL_R * CANVAS_SCALE * 3:   # must click reasonably close
            target_ball_var.set(best_idx)
            _redraw()
    ...
    # Target ball selection hint
    ttk.Label(ctrl, text="Target Ball (click on canvas):").pack(anchor="w", pady=(0, 2))
    info_target_ball_label = ttk.Label(ctrl, text="Ball: 0", foreground="#e8c33a")
    info_target_ball_label.pack(anchor="w", padx=8, pady=(0, 4))
    ...
    ttk.Label(ctrl, text="Hit Strength:").pack(anchor="w", pady=(0, 4))
    hit_var = tk.IntVar(value=2)  # default: Mid-Heavy

    hit_radio_frame = tk.Frame(ctrl, bg=CLR_PANEL)
    hit_radio_frame.pack(anchor="w", padx=8, pady=(0, 4))

    for label_text, level, color in HIT_LEVELS:
        rb = tk.Radiobutton(
            hit_radio_frame,
            text=label_text,
            variable=hit_var,
            ...
            font=("Helvetica", 10),
            command=lambda c=color: _update_hit_bar(c),
        )
        rb.pack(anchor="w", pady=1)

    # Colour bar showing selected level
    hit_bar = tk.Canvas(ctrl, width=200, height=12, bg=CLR_PANEL,
                        highlightthickness=0)
    hit_bar.pack(anchor="w", padx=8, pady=(2, 0))

    def _update_hit_bar(color=None):
        hit_bar.delete("all")
        if color is None:
            color = HIT_LEVELS[hit_var.get()][2]
        filled = (hit_var.get() + 1) / len(HIT_LEVELS)
        hit_bar.create_rectangle(0, 0, 200, 12, fill="#333355", outline="")
        hit_bar.create_rectangle(0, 0, int(200 * filled), 12, fill=color, outline="")

    _update_hit_bar()  # initialise bar

    hit_strength_label = ttk.Label(ctrl, text="")
    hit_strength_label.pack(anchor="w", padx=8)

    def _update_hit_strength_label(*_):
        names = ["Soft", "Mid", "Mid-Heavy", "Heavy"]
        hit_strength_label.configure(
            text=f"Power: {names[hit_var.get()]}")

    _update_hit_strength_label()
    hit_var.trace_add("write", lambda *_: [_update_hit_strength_label(), _update_hit_bar()])
    ...
    def _on_confirm():
        ...
        result.update({
            ...
            "score": 0,
            "obstacle": obstacle,
            "hit_strength": hit_var.get(),  # 0=Soft 1=Mid 2=Mid-Heavy 3=Heavy
        })
        root.destroy()
    ...
    def _compute_vector(pot_idx, offset_deg):
        ...
        tidx = target_ball_var.get()
        aim_x = AIM_X[pot_idx]
        aim_y = AIM_Y[pot_idx]

        # direction from target ball toward aim-point
        target_vx = aim_x - object_ball_x[tidx]
        target_vy = aim_y - object_ball_y[tidx]

        # hitpoint on the target ball surface (opposite to aim direction)
        hp_x, hp_y = strat.findhitpoint(
            object_ball_x[tidx], object_ball_y[tidx], target_vx, target_vy)
    ...
    def _compute_trajectory(pot_idx, offset_deg):
        ...
        if adj_vlen < 1e-6:
            return [(cue_x, cue_y)], [(object_ball_x[tidx], object_ball_y[tidx])], [], (cue_x, cue_y)
        ...
        dx = object_ball_x[tidx] - cue_x
        dy = object_ball_y[tidx] - cue_y
        ...
        impact_vx = object_ball_x[tidx] - cue_contact_x
        impact_vy = object_ball_y[tidx] - cue_contact_y
        ...
        target_path = _trace_with_bounces(
            object_ball_x[tidx], object_ball_y[tidx], impact_ux, impact_uy, 600)
    ...
    def _redraw():
        ...
        # -- Target ball highlight ring
        tidx = target_ball_var.get()
        if 0 <= tidx < len(object_ball_x):
            tbx, tby = a2c(object_ball_x[tidx], object_ball_y[tidx])
            tbr = BALL_R * CANVAS_SCALE + 6
            canvas.create_oval(tbx - tbr, tby - tbr, tbx + tbr, tby + tbr,
                                fill="", outline="#e8c33a", width=3)
        ...
        info_target_ball_label.configure(text=f"Ball: {target_ball_var.get()}")
    ...
    if not result:
        # fallback: straight shot at pot 0, mid-heavy strength
        ...
        result.update({
            ...
            "score": 0,
            "obstacle": obstacle,
            "hit_strength": 2,  # default Mid-Heavy
        })
