#!/usr/bin/env python3
"""
Hiwin Billiards Controller + Shot Planner UI (Fixed)
...
import hiwin_control.hiwin_control.nine_ball_strat_old as table2
...
def check_mid_pose(all_ball_pose):
    errors = [math.hypot(all_ball_pose[i]-960, all_ball_pose[i+1]-540)
              for i in range(0, len(all_ball_pose), 2)]
    idx = errors.index(min(errors))
    return all_ball_pose[2*idx], all_ball_pose[2*idx+1]

def mid_point_error(mid_ball):
    return math.hypot(mid_ball[0]-960, mid_ball[1]-540)

def pixel_mm_convert(cam_h, pixels):
    fov_x, fov_y = 34.5, 21.0          # half-FOV degrees
    px, py = 960, 540
    cx = cam_h / math.tan(math.radians(90 - fov_x))
    cy = cam_h / math.tan(math.radians(90 - fov_y))
    return [cx/px*(pixels[0]-px), cy/py*(pixels[1]-py), 1.0]
...
class Hiwin_Controller(Node):
    ...
    def __init__(self):
        ...
        self.all_label        = []
        self.data_flag        = 0
        self._yolo_active     = False   # only update positions in LOCK_INFO / DYNAMIC_CALI
        ...
        self.get_logger().info('Hiwin Controller ready')
    ...
    def _sm(self, state):
        if state == States.INIT:
            ...
            req = self._req(cmd_type=RobotCommand.Request.JOINTS_CMD,
                            joints=END_TURN_RIGHT)
            res = self._call(req)
            if res is None:
                self.get_logger().warn('Home move timed out — skipping to MOVE_TO_PHOTO')
            ...
            self.get_logger().info(f'Shot received: x={arm_x:.1f} y={arm_y:.1f}')
...
class BilliardsUI:
    ...
    def _build_ui(self):
        ...
        self.btn_send   = big_btn('SEND TO ROBOT ▶',  '#27ae60', 21, self._send_shot,
                                  state='disabled')
    ...
    def _freeze_balls(self):
        ...
        self._set_state('SELECT TARGET', 'Click the ball you want to hit', '#f39c12')
        self._draw()
    ...
    def _calculate_shot(self):
        ...
        self._set_state('PREVIEW', 'Review then click SEND TO ROBOT', '#2980b9')
    ...
    def _send_shot(self):
        if self.ui_state != UI_PREVIEW:
            return
        if not self.white_ball:
            messagebox.showerror('No white ball', 'Cannot find white ball position.')
            return
        wx, wy = self.white_ball['x'], self.white_ball['y']
        # Hit point is the white ball — arm approaches along shot angle
        arm_x, arm_y = canvas_to_arm_mm(wx, wy)

        # Pass YOLO pixel coords so LOCK_INFO can re-detect the correct balls
        def _canvas_to_yolo(cx, cy):
            ax, ay = canvas_to_arm_mm(cx, cy)
            ix, iy = _apply_h(np.linalg.inv(_H_IMG_TO_ARM), ax, ay)
            return [ix, iy]

        white_yolo  = _canvas_to_yolo(wx, wy)
        target_yolo = (_canvas_to_yolo(self.target_ball['x'], self.target_ball['y'])
                       if self.target_ball else None)

        self.node.set_shot_from_ui(arm_x, arm_y, self.best_angle, self.v_force.get(),
        ...
        self.status['text'] = (f'Shot sent → arm({arm_x:.1f}, {arm_y:.1f}) '
                               f'angle={self.best_angle:.1f}° force={self.v_force.get()}')
                ...
                if (b['label'].lower() not in ('white', 'cue', 'w') and
                    ...
                    if self.target_pocket:
                        ...
                        self._set_state('SELECT POCKET',
                                        'Choose pocket in the panel →', '#9b59b6')
                    self._draw(); return
        # drag white ball
        if (self.ui_state in (UI_PREVIEW, UI_SELECT) and self.white_ball and
                math.hypot(mx-self.white_ball['x'], my-self.white_ball['y']) <= BR+4):
...
def main(args=None):
    ...
    # ROS spin in background thread — ONE executor, no conflicts
