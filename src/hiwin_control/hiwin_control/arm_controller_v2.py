...
import hiwin_control.config_utils as config_utils
import hiwin_control.pool_aim_ui as pool_aim_ui
...
HIT_APPROACH_VELOCITY = 35
HIT_APPROACH_ACCELERATION = 35

LIGHT_PIN = 6
HITSOFT_PIN = 4
HITMID_PIN = 5
HITMID_HEAVY_PIN = 3   # mid-heavy level
HITHEAVY_PIN = 1
HEAVY_PIN = 2
...
file_path_ini = str(config_utils.resolve_config_path('eye_in_hand_calibration.ini'))
...
file_path_yaml = str(config_utils.resolve_config_path('arm.yaml'))
...
def check_mid_pose(all_ball_pose):
    if not all_ball_pose:
        return 1920 / 2, 1080 / 2   # no detection — return frame centre
    mid_error = []
...
class Hiwin_Controller(Node):
    ...
    def yolo_callback(self, msg):
        if not msg.data:
            self.all_ball_pose = []   # clear stale data so old balls don't linger
            self.data_flag = 0
        else:
    ...
    def _state_machine(self, state: States) -> States:
        if state == States.INIT:
            ...
            self.fix_check_point = []
            self.get_logger().info('MOVING TO LEFT PHOTO POSE...')
            req = self.generate_robot_request(
                cmd_mode = RobotCommand.Request.DIGITAL_OUTPUT,
                digital_output_cmd = RobotCommand.Request.DIGITAL_ON,
                digital_output_pin = LIGHT_PIN
            ...
            elif self.fix_check_point == FIX_ABS_RIGHT_CAM:
                print("White not found at right, trying left...")
                nest_state = States.FIX_LEFT_PHOTO_POSE

            else:
                # Not found at left either — restart from center and keep searching
                print("White not found at left, restarting search from center...")
                nest_state = States.MOVE_TO_PHOTO_POSE

        elif state == States.LOCK_INFO:
            self.ball_pose = []
            self.get_logger().info('WAITING FOR FRESH BALL DETECTION...')
            # Force-wait for a new YOLO frame captured after the arm settled.
            # Without this, all_ball_pose may still hold data from the previous
            # round (ball 1 already pocketed), causing DYNAMIC_CALI to move to
            # an empty pocket and hang.
            self.data_flag = 0
            t_start = time.time()
            while self.data_flag == 0:
                if time.time() - t_start > 5.0:
                    self.get_logger().warn('Timeout waiting for ball detection at photo pose')
                    break
                time.sleep(0.05)
            time.sleep(0.3)   # small settle after first detection frame
            ...
            if self.index < len(self.target_cue):
                ...
                if res.arm_state == RobotCommand.Response.IDLE and self.index < len(self.target_cue):
                    ...
                    nest_state = States.USER_AIM

        elif state == States.USER_AIM:
            self.get_logger().info('LAUNCHING USER AIM UI...')
            # Convert all detected ball positions to arm coordinates
            actual_x = []
            actual_y = []
            ...
            # Override calibrated target and cue positions
            ...
            # Launch the interactive UI (blocking)
            ui_result = pool_aim_ui.show_aim_ui(
                object_ball_x=actual_x[:-1],
                object_ball_y=actual_y[:-1],
                cue_x=cuex,
                cue_y=cuey,
            )
            self.get_logger().info(f'User selected pot {ui_result["pot"]}, '
                                   f'vector=({ui_result["vx"]:.1f}, {ui_result["vy"]:.1f})')

            # Store as strategy_info format: [score, vx, vy, obstacle, hitpoint_x, hitpoint_y, hit_strength]
            self.strategy_info = [
                ui_result['score'],
                ui_result['vx'],
                ui_result['vy'],
                ui_result['obstacle'],
                ui_result['hitpoint_x'],
                ui_result['hitpoint_y'],
                ui_result.get('hit_strength', 2),  # 0=Soft 1=Mid 2=Mid-Heavy 3=Heavy
            ]
            self.user_aim_override = True
            nest_state = States.OPEN_SEC_IO
            ...
            # If user already selected aim via UI, skip auto strategy
            if getattr(self, 'user_aim_override', False):
                self.get_logger().info('USING USER-SELECTED AIM (skipping auto strategy)')
                print("strategy info (user override):", self.strategy_info)
                self.user_aim_override = False
            ...
            req = self.generate_robot_request(
                cmd_mode = RobotCommand.Request.PTP,
                tool = CUE_TOOL,
                velocity = HIT_APPROACH_VELOCITY,
                acceleration = HIT_APPROACH_ACCELERATION,
                pose = pose
            )
            res = self.call_hiwin(req)
            if res.arm_state == RobotCommand.Response.IDLE:
            ...
            # ── Determine hit pin from hit_strength (index 6 of strategy_info) ──
            # hit_strength: 0=Soft, 1=Mid, 2=Mid-Heavy, 3=Heavy
            # Fallback to score-based selection when user override is not active
            if len(self.strategy_info) > 6:
                # User-chosen strength from UI
                strength = self.strategy_info[6]
                hitpin = [HITSOFT_PIN, HITMID_PIN, HITMID_HEAVY_PIN, HITHEAVY_PIN][strength]
            else:
                # Auto-strategy score-based selection
                if self.score <= 0:
                    hitpin = HITHEAVY_PIN
                elif self.score <= 2000:
                    hitpin = HITMID_HEAVY_PIN
